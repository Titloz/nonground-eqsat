...
pub(crate) fn deduct(wo: &mut VecDeque<Class>, us: &mut VecDeque<Class>, c0: Class) -> bool {
    ...
    if !deduct_intern(wo, us, f0, f1, 0, 1, &c0, &mut Class::new(), false) {
        pop_value(wo, &c0);
        return false;
    }
    ...
    if !deduct_intern(wo, us, g0, g1, 0, 1, &c0, &mut Class::new(), false) {
        pop_value(wo, &c0);
        return false;
    }
    ...
    if !deduct_intern(wo, us, h0, h1, 0, 1, &c0, &mut Class::new(), false) {
        pop_value(wo, &c0);
        return false;
    }
    true
