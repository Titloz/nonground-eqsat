...
pub(crate) fn deduct_intern(wo: &mut VecDeque<Class>, us: &mut VecDeque<Class>, t0: Term, t1: Term, i: usize, n: usize, c0: &Class, c_new: &mut Class, used: bool) -> bool {
    if i!=n {
        for c in wo.clone() {
            let cbis = &c.clone();
            let cter = &c.clone();
            ...
            for s0 in &cter.terms {
                for s1 in &cquadr.terms {
                    ...
                    // t0 = t0[s0]_i
                    // t1 = t1[s1]_i
                    if !deduct_intern(wo, us, t0.clone(), t1.clone(), i+1, n, c0, c_new, (c == *c0) || used) {
                        return false;
                    }
                }
            }
